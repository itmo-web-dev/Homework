...
A.bonus=[10, 20, 30];
A.bonus.tupic = 100;
var B=Math_sd_learn.clone_array(A);
console.log(A === B);
A.push(3);
console.log(A);
console.log(B);
...
describe("task_03 Math_sd_learn.min", function(){
    var res_correct, msg;
    var value_in = [3,6,7,8];
    res_correct=3;
    msg = "минимальное значение из {0}: {1}".replace('{1}',res_correct).replace('{0}',value_in);
    it(msg, function(){
       expect(Math_sd_learn.min(value_in)).toEqual(res_correct);
    });
 });      
 describe("task_03 Math_sd_learn.min", function(){
    var res_correct, msg;   
...
describe("task_03 Math_sd_learn.max", function(){
    var res_correct, msg;
    var value_in = [3,6,7,8];
    res_correct=8;
    msg = "максимальное значение из {0}: {1}".replace('{1}',res_correct).replace('{0}',value_in);
    it(msg, function(){
       expect(Math_sd_learn.max(value_in)).toEqual(res_correct);
    });
 });    
describe("task_03 Math_sd_learn.max", function(){
    var res_correct, msg;    
...
describe("task_03 Math_sd_learn.average", function(){
    var res_correct, msg;    
    var value_in = [-10, 6,7,8];
    res_correct= 2.75;
    msg = "максимальное значение из {0}: {1}".replace('{1}',res_correct).replace('{0}',value_in);
    it(msg, function(){
        expect(Math_sd_learn.average(value_in)).toEqual(res_correct);
    });
});    
